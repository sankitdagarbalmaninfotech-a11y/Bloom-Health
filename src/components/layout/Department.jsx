    ...
    <div className='bg-gray-50 min-h-screen'>
      ...
      <div className='flex flex-col lg:flex-row px-6 lg:px-20 py-8 gap-8'>
        ...
        <div className='w-full lg:w-[70%] space-y-8'>
          ...
          {/* Services Provided */}
          <section>
            <h3 className='text-4xl font-semibold text-blue-500 mb-4'>Services Provided</h3>
            <div className='space-y-2'>
              {services.map((service, index) => (
                <button 
                  key={index}
                  className='w-full text-left px-4 py-2 bg-white border border-gray-300 rounded-md shadow hover:bg-gray-100'
                >
                  {service}
                </button>
              ))}
          ...
          <section>
            <h3 className='text-4xl font-semibold text-blue-500 mb-4'>Diseases & Conditions Treated</h3>
            <div className='flex flex-row'>
              <ul className='text-gray-700 space-y-1 w-[50%]'>
                {diseases.slice(0, Math.ceil(diseases.length/2)).map((disease, index) => (
                  <li key={index} className='flex items-center'>
                    <CheckCircleOutlineOutlinedIcon fontSize='small' className='text-black mr-2' />
                    {disease}
                  </li>
                ))}
              </ul>
              ...
              <ul className='text-gray-700 space-y-1 w-[50%]'>
                {diseases.slice(Math.ceil(diseases.length/2)).map((disease, index) => (
                  <li key={index} className='flex items-center'>
                    <CheckCircleOutlineOutlinedIcon fontSize='small' className='text-black mr-2' />
                    {disease}
                  </li>
          ...
          <section className='bg-gray-50 space-y-4'>
            <div className='grid gap-4'>
              {faqs.map((faq, index) => (
                <div
                  key={index}
                  className="bg-white rounded-lg shadow-md p-3 cursor-pointer transition transform duration-300 hover:scale-105 hover:bg-blue-50"
                >
                  <button
